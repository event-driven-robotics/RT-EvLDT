...
private:
    ...
    std::thread events_process, detection_process, tracking_process, laptopcam_process;
    bool streaming{false};
    fs::path vout_path;
    double fit_thresh{0.2}, inlier{0.2}, alpha{1.0}, display_fps{30}, dp{1.1};
    int block_size{14};
    cv::Size res, savesize, savesize_laptop;
    bool line_track{false}, use_laptop_cam{false};
    std::string window_name;
    cv::Mat disp_img, laptop_frame, tmp_laptop_frame, laptop_frame_sized, output;
    cv::VideoWriter vw, vw_laptop;
    std::mutex laptop_frame_mutex;

public:
    bool configure(yarp::os::ResourceFinder &rf) override {
        if (rf.check("help")) {
            yInfo() << "LEDGE online with laptop camera for recording";
            ...
            yInfo() << "--laptop_cam          \t: Flag to use laptop camera";
            return false;
        }
        ...
        use_laptop_cam = rf.check("laptop_cam");
        savesize = {res.width * 2, res.height}; // Put 2 images horizontally
        display_fps = 30;

        // Initialize recorded video for ledge
        fs::path defaultpath = fs::path(std::getenv("HOME")) / fs::path("Downloads/ledge-online-laptopcam.mp4");
        ...
        cv::resizeWindow(window_name, savesize);
        cv::VideoCapture cap(0);
        savesize_laptop = {static_cast<int>(cap.get(cv::CAP_PROP_FRAME_WIDTH)), static_cast<int>(cap.get(cv::CAP_PROP_FRAME_HEIGHT))};
        laptop_frame = cv::Mat::zeros(savesize_laptop, CV_8UC3);
        output = cv::Mat::zeros(savesize, CV_8UC3);
        cap.release();
        ...
        if (use_laptop_cam) {
            laptopcam_process = std::thread([this] { this->captureLaptopCam(); });
        }
    ...
    void captureLaptopCam() {
        cv::VideoCapture cap(0);
        if (!cap.isOpened()) {
            std::cerr << "Could not open your laptop camera..." << std::endl;
            return;
        }
        while (!isStopping()) {
            if (cap.read(tmp_laptop_frame)) {
                std::lock_guard<std::mutex> lock(laptop_frame_mutex);
                laptop_frame = tmp_laptop_frame.clone(); // Thread safe
            }
        }
    }
    ...
    bool updateModule() override {
        ...
        cv::imshow("Laptop camera", laptop_frame);

        // Recreate recorded frame
        cv::Mat left_output = output(cv::Rect(0, 0, savesize.width / 2, savesize.height));
        cv::Mat right_disp = disp_img(cv::Rect(savesize.width / 2, 0, savesize.width / 2, savesize.height));
        right_disp.copyTo(left_output);
        cv::Mat right_output = output(cv::Rect(savesize.width / 2, 0, savesize.width / 2, savesize.height));
        cv::resize(laptop_frame, laptop_frame_sized, {savesize.width / 2, savesize.height});
        laptop_frame_sized.copyTo(right_output);
